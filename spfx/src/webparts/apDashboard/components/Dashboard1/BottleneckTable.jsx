...
export default function BottleneckTable({ data, selectedApprover, onSelectApprover }) {
  const { summary, avgDays } = useMemo(() => {
    // Overall average days in workflow across all invoices
    const avgDays = data.length
      ? Math.round(data.reduce((s, d) => s + d.DAYS_IN_WORKFLOW, 0) / data.length)
      : 0;
    ...
    data.forEach((d) => {
      ...
      a.totalDays  += d.DAYS_IN_WORKFLOW;
      a.maxDays     = Math.max(a.maxDays, d.DAYS_IN_WORKFLOW);
      if (d.DISCOUNT_AT_RISK) a.atRiskCount += 1;
    });

    // Only include approvers whose average days exceeds the overall average
    const all = Object.values(map).map((a) => ({ ...a, avgDays: Math.round(a.totalDays / a.count) }));
    const filtered = all.filter((a) => a.avgDays > avgDays);
    return {
      summary: filtered.sort((a, b) => b.avgDays - a.avgDays || b.count - a.count),
      avgDays,
    };
  }, [data]);
  ...
  return (
    <div>
      <SectionHeader
        ...
        subtitle={`Approvers above average — overall avg: ${avgDays}d · click a row to filter`}
      />

      {summary.length === 0 ? (
        <p className="py-6 text-center text-sap-subtext text-xs">
          No approvers are above the average workflow time of {avgDays}d.
        </p>
      ) : (
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-sap-border">
                ...
                <th className="text-left py-2 px-3 font-semibold text-sap-subtext uppercase tracking-wide w-44">Total Value</th>
                <th className="text-center py-2 px-3 font-semibold text-sap-subtext uppercase tracking-wide">Avg Age</th>
            ...
            <tbody>
              {summary.map((row, i) => {
                ...
                const aboveAvgBy = row.avgDays - avgDays;
                return (
                  <tr
                    key={row.id}
                  ...
                  >
                    ...
                    <td className="py-2.5 px-3 text-center font-bold text-sap-text">{row.count}</td>
                    <td className="py-2.5 px-3">
                      <BarIndicator value={row.totalValue} max={maxVal} />
                    </td>
                    <td className="py-2.5 px-3 text-center">
                      <span className="font-semibold text-red-600">{row.avgDays}d</span>
                      <span className="text-sap-subtext ml-1">(+{aboveAvgBy}d)</span>
                    </td>
                    <td className="py-2.5 px-3 text-center">
                      <span className="font-medium text-sap-subtext">{row.maxDays}d</span>
                    </td>
