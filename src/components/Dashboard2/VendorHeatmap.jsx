...
export default function VendorHeatmap({ vendors }) {
  const maxGR  = Math.max(...vendors.map((v) => v.GR_WITHOUT_IR));
  const maxIR  = Math.max(...vendors.map((v) => v.IR_WITHOUT_GR));

  return (
    <div>
      <SectionHeader
        title="Vendor Open Receipt Heatmap"
        subtitle="Top vendors by open GR/IR balance — darker = higher value"
      />
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-sap-border">
              ...
              <th className="text-center py-2 px-2 font-semibold text-red-700 uppercase tracking-wide">GR w/o IR</th>
              <th className="text-center py-2 px-2 font-semibold text-amber-700 uppercase tracking-wide">IR w/o GR</th>
          ...
          <tbody>
            {vendors.map((v, i) => (
              <tr
                key={v.LIFNR}
                className={`border-b border-sap-border ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}
              >
                ...
                <td className="py-2 pl-2 text-right font-semibold text-sap-text">
                  {fmt(v.TOTAL_OPEN)}
                </td>
              </tr>
            ))}
